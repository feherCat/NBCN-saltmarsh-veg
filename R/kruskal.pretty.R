kruskal.pretty = function(otu, metrics, csv, interesting, output_name){
  ...
  R_abund_sd=c()
  abund=as.matrix(otu)
  #abund=abund/rowSums(abund) # I think this is not needed since I already converted to relative % cover
  for(b in unique(csv$Comparison)){
    ...
    for(otus in otu_list$OTU){
      ...
      for(sample in sample_L){
        L=append(L,abund[sample,str_remove(otus, " ")]) 
        #L=append(L,abund[as.numeric(sample),otus]) # sample needed to be converted to numeric
        mean_L[[otus]]=mean(as.numeric(L))
        sd_L[[otus]]=sd(as.numeric(L))
      }
      for(sample in sample_R){
        R=append(R,abund[sample,str_remove(otus, " ")]) 
        #R=append(R,abund[as.numeric(sample),otus]) # sample needed to be converted to numeric
    ...
    for(otus in otu_list$OTU){
      otus = as.character(otus)
      result=kruskal.test(listbact[[str_remove(otus, " ")]]~listmet[[topic1]])
  ...
  o_csv[,7]=tax
  #o_csv[,8]=L_mean
  o_csv[,8]=as.numeric(L_mean) # these columns need to be numeric
  #o_csv[,9]=L_sd
  ...
  colnames(o_csv)[which(names(o_csv) == "V7")] <- "taxonomy" # make names syntactically valid
